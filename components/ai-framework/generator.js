...
export function AIGenerator({
    ...
    const actualMode = wp.hooks.applyFilters('ai_generator_mode', mode, { type, context });
    ...
    const generate = async (customPrompt = null) => {
        ...
        try {
            ...
            const endpoint = wp.hooks.applyFilters('ai_generator_endpoint', '/polaris/v1/ai/generate');
            ...
            if (response.text) {
                ...
                if (!validation.valid) {
                    throw new Error(validation.error || __('Validation failed', 'wp-component-library'));
                }
                ...
                processed = wp.hooks.applyFilters('ai_generator_output', processed, {
                    type,
                    context,
                    raw: response.text
                });
                ...
                onChange(processed);
                wp.hooks.doAction('ai_generator_success', { type, value: processed });
            }
        } catch (err) {
            const errorMessage = err.message || __('Generation failed', 'wp-component-library');
    ...
    if (actualMode === 'button') {
        // Determine button text - priority: prop > type config > default
        const displayText = isGenerating 
            ? __('Generating...', 'wp-component-library')
            : (buttonText || typeConfig.buttonLabel || __('Generate', 'wp-component-library'));
        
        // Determine icon - priority: prop > type config > none
        const displayIcon = icon !== undefined ? icon : typeConfig.buttonIcon;
    ...
    if (actualMode === 'prompt') {
        return (
            <div className={`ai-generator-prompt ${className || ''}`}>
                <TextareaControl
                    placeholder={typeConfig.promptPlaceholder || __('Describe what you want to generate...', 'wp-component-library')}
                    value={promptValue}
    ...
    return (
        <AIGenerator {...props} type={type} value={value} onChange={onChange} mode="button" context={context} />
    );
}